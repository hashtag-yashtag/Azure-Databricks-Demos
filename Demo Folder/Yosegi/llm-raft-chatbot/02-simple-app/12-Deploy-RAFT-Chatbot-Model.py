...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=data-science&org_id=984752964297111&notebook=%2F02-simple-app%2F02-Deploy-RAG-Chatbot-Model&demo_name=llm-rag-chatbot&event=VIEW&path=%2F_dbdemos%2Fdata-science%2Fllm-rag-chatbot%2F02-simple-app%2F02-Deploy-RAG-Chatbot-Model&version=1">
...
raft_chain_config = {
    "databricks_resources": {
        "llm_endpoint_name": "yosegi_fine_tuned_dbsupport",
...
try:
    with open('raft_chain_config.yaml', 'w') as f:
        yaml.dump(raft_chain_config, f)
except:
    print('pass to work on build job')
model_config = mlflow.models.ModelConfig(development_config='raft_chain_config.yaml')

# COMMAND ----------

# DBTITLE 1,Write the chain to a companion file to avoid serialization issues
# MAGIC %%writefile raft_chain.py
# MAGIC import os
# MAGIC import mlflow
...
# MAGIC model_config = mlflow.models.ModelConfig(development_config='raft_chain_config.yaml')
# MAGIC
...
# MAGIC mlflow.models.set_model(model=raft_chain)
# MAGIC
# MAGIC # COMMAND ----------
# MAGIC
# MAGIC # raft_chain.invoke(model_config.get("input_example"))

# COMMAND ----------

# Log the model to MLflow
with mlflow.start_run(run_name=f"yosegi_raft_quickstart"):
    logged_chain_info = mlflow.langchain.log_model(
        lc_model=os.path.join(os.getcwd(), 'raft_chain.py'),  # Chain code file e.g., /path/to/the/chain.py 
        model_config='raft_chain_config.yaml',  # Chain configuration 
        artifact_path="raft_chain",  # Required by MLflow
...
# cleanup_demo(catalog, db, "agents_yosegi_demo-dbdemos_rag_chatbot-basic_rag_demo", f"{catalog}.{db}.databricks_documentation_vs_index")
