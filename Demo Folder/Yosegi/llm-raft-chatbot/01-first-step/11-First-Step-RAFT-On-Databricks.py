...
# MAGIC %md-sandbox
# MAGIC
# MAGIC # 1/ Deploying our first RAFT application with Mosaic AI Agent Framework & Agent Evaluation
# MAGIC
...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=data-science&org_id=984752964297111&notebook=%2F01-first-step%2F01-First-Step-RAG-On-Databricks&demo_name=llm-rag-chatbot&event=VIEW&path=%2F_dbdemos%2Fdata-science%2Fllm-rag-chatbot%2F01-first-step%2F01-First-Step-RAG-On-Databricks&version=1">
...
# MAGIC %run ../_resources/00-init-ft $reset_all_data=false
...
chain_config = {
    "llm_model_serving_endpoint_name": "yosegi_fine_tuned_dbsupport",  # the foundation model we want to use
...
# MAGIC
# MAGIC ## 2.3/ Building Databricks Chat Model to query a Custom Fine-Tuned model
# MAGIC
...
# MAGIC - Your fined-tune model **(what we'll use)**
...
prompt = ChatPromptTemplate.from_messages(
    [  
        ("user", model_config.get("llm_prompt_template") + "\n" + "{question}") # Contains the instructions from the configuration and user's questions
    ]
)
...
with mlflow.start_run(run_name="basic_raft_bot"):
  logged_chain_info = mlflow.langchain.log_model(
          #Note: In classical ML, MLflow works by serializing the model object.  In generative AI, chains often include Python packages that do not serialize.  Here, we use MLflow's new code-based logging, where we saved our chain under the chain notebook and will use this code instead of trying to serialize the object.
          lc_model=os.path.join(os.getcwd(), 'RAFT_chain'),  # Chain code file e.g., /path/to/the/chain.py 
...
MODEL_NAME = "basic_raft_demo"
MODEL_NAME_FQN = "yosegi_demo.dbdemos_raft_chatbot.basic_raft_demo"
