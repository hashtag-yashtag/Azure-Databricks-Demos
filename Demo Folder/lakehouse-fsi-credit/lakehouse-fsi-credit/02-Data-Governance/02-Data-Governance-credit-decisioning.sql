...
-- MAGIC To run this demo, just select the cluster `dbdemos-lakehouse-fsi-credit-yash_gupta` from the dropdown menu ([open cluster configuration](https://adb-984752964297111.11.azuredatabricks.net/#setting/clusters/0607-151116-q6rh19a6/configuration)). <br />
...
-- MAGIC %run ../_resources/00-setup $reset_all_data=false $catalog=yash_gupta_fsi_credit $db=yash_gupta_fsi_credit

-- COMMAND ----------

--CREATE CATALOG IF NOT EXISTS yash_gupta_fsi_credit;
--USE CATALOG yash_gupta_fsi_credit;
SELECT CURRENT_CATALOG();

-- COMMAND ----------

-- DBTITLE 1,As you can see, our tables are available under our catalog.
CREATE SCHEMA IF NOT EXISTS yash_gupta_fsi_credit;
...
GRANT SELECT ON TABLE yash_gupta_fsi_credit.yash_gupta_fsi_credit.credit_bureau_gold TO `analysts`;
GRANT SELECT ON TABLE yash_gupta_fsi_credit.yash_gupta_fsi_credit.customer_gold TO `analysts`;
GRANT SELECT ON TABLE yash_gupta_fsi_credit.yash_gupta_fsi_credit.fund_trans_gold TO `analysts`;

-- We'll grant an extra MODIFY to our Data Engineer
GRANT SELECT, MODIFY ON SCHEMA yash_gupta_fsi_credit.yash_gupta_fsi_credit TO `dataengineers`;
...
CREATE OR REPLACE VIEW  yash_gupta_fsi_credit.yash_gupta_fsi_credit.customer_gold_secured AS
SELECT
  c.* EXCEPT (first_name),
  CASE
  ...
  END AS first_name
FROM
  yash_gupta_fsi_credit.yash_gupta_fsi_credit.customer_gold_features AS c;
...
SELECT cust_id, first_name FROM yash_gupta_fsi_credit.yash_gupta_fsi_credit.customer_gold_secured;
...
-- ALTER SHARE dbdemos_credit_decisioning_customer  ADD TABLE yash_gupta_fsi_credit.yash_gupta_fsi_credit.credit_bureau_gold ;
