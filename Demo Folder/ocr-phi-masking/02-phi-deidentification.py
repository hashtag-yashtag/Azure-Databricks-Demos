...
dbutils.fs.ls(f"dbfs:{util.delta_path}/silver/obfuscated_notes")

# COMMAND ----------

# List all files in the specified directory
dbutils.fs.ls("dbfs:/home/yash.gupta@databricks.com/hls/phi_ocr/delta/silver/obfuscated_notes")

# COMMAND ----------

# Read the Delta file into a DataFrame
file_path = "dbfs:/home/yash.gupta@databricks.com/hls/phi_ocr/delta/silver/obfuscated_notes/"
...
# Convert each row's "deidentified_notes" to a PDF and save it
pdf_paths = []
for row in df.select("deidentified_notes").collect():
    text = row["deidentified_notes"]
    output_path = f"/dbfs/home/yash.gupta@databricks.com/hls/phi_ocr/{hash(text)}.pdf"
    text_to_pdf(text, output_path)
    pdf_paths.append(output_path)

# Create a DataFrame with the PDF paths
pdf_df = spark.createDataFrame([(path,) for path in pdf_paths], ["pdf_path"])

# Display the DataFrame with PDF paths
display(pdf_df)
